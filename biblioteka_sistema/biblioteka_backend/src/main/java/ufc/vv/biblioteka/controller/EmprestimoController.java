...
public class EmprestimoController {
    ...
    public ResponseEntity<?> emprestarLivro(int livroId, @RequestParam int leitorId, @RequestParam String senha) {
        try {
    ...
    @PostMapping("/renovar")
    public ResponseEntity<?> renovarEmprestimo(@RequestParam int emprestimoId, @RequestParam String senha) {
        try {
            Emprestimo emprestimo = emprestimoService.renovarEmprestimo(emprestimoId, senha);
            return ResponseEntity.ok(emprestimo);
        } catch (EntityNotFoundException e) {
            return ResponseEntity.notFound().build();
        } catch (LimiteExcedidoException | DataRenovacaoException | ReservaEmEsperaException e) {
            return ResponseEntity.status(HttpStatus.CONFLICT).body(e.getMessage());
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("Erro inesperado ao devolver livro.");
        }
